...
class RunPanel {
    ...
    static show(watcher, runner) {
        ...
        const panel = vscode.window.createWebviewPanel('babelgitRun', `Review: ${wi.id}`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
...
function getLockedCommit(cwd) {
    try {
        return (0, child_process_1.execSync)('git rev-parse --short HEAD', { cwd, encoding: 'utf8' }).trim();
    }
    catch {
        return 'unknown';
    }
...
function getDiffStat(cwd) {
    try {
        ...
        const stat = (0, child_process_1.execSync)('git diff --stat HEAD~1..HEAD 2>/dev/null || git diff --stat HEAD', {
...
function buildHtml(wi, lockedCommit, diffStat) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
