...
import { StateWatcher, WorkItemState, CheckpointState, CheckpointGroup } from './stateWatcher'

export class BoardPanel {
  private static instance: BoardPanel | undefined
  ...
  private constructor(private watcher: StateWatcher) {
    ...
    this.disposables.push(
      watcher.onDidChange(() => this.render(watcher))
    )
  ...
  private render(watcher: StateWatcher): void {
  ...
  private handleMessage(msg: { type: string; command?: string; args?: unknown[]; id?: string }): void {
...
function buildColumns(
  ...
  const COLS: Array<{ stage: string; label: string; color: string; borderColor: string }> = [
...
function cardActions(wi: WorkItemState, currentId: string | undefined): string {
  const stage = (wi.stage === 'in_progress' && wi.ship_ready) ? 'ship_ready' : wi.stage
  const isCurrent = wi.id === currentId
  const id = esc(wi.id)

  const btn = (label: string, command: string, args: string, cls = '') =>
    `<button class="action-btn ${cls}" onclick="cmd('${command}', ${args})">${label}</button>`
...
function buildColumn(col: Column, groups: CheckpointGroup[], currentId: string | undefined): string {
